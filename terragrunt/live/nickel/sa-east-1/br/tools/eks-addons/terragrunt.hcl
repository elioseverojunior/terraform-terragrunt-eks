...
generate "provider-github" {
  ...
  contents  = <<-EOF
    provider "github" {
      owner = "${include.root.locals.inputs.github_owner}"
    }
  EOF
}
    ...
    extra_values                = <<-EXTRA_VALUES
      grafana:
        ...
        ingress:
          ...
          enabled: true
          hosts:
            - telemetry.${include.root.locals.inputs.default_domain_name}
          tls:
            - secretName: ${include.root.locals.inputs.default_domain_name}
              hosts:
                - telemetry.${include.root.locals.inputs.default_domain_name}
        persistence:
          enabled: true
  ...
  loki-stack = {
    ...
    extra_values         = <<-VALUES
      ...
      ingress:
        ...
        hosts:
          - host: logz.${include.root.locals.inputs.default_domain_name}
            paths: ["/"]
        tls:
          - secretName: logz.${include.root.locals.inputs.default_domain_name}
            hosts:
              - logz.${include.root.locals.inputs.default_domain_name}
        VALUES
